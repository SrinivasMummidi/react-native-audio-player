...
const SearchInput: React.FC<SearchInputProps> = ({
    ...
    const { searchState, search, clearSearch, nextMatch, previousMatch } = useSearch();
    ...
    return (
        <View style={[styles.container, hasSearchTerm && styles.containerWithResults, style]}>
                ...
                <View style={styles.resultsContainer}>
                    <View style={styles.resultsInfo}>
                        <Text style={styles.resultsText}>
                            {searchState.isSearching
                                ? 'Searching...'
                                : hasResults
                                    ? `${searchState.matches.length} result${searchState.matches.length !== 1 ? 's' : ''}`
                                    : 'No results'
                            }
                        </Text>
                    </View>
                    ...
                    {hasResults && (
                        <View style={styles.navigationContainer}>
                            <TouchableOpacity
                                style={[styles.navButton, !hasResults && styles.navButtonDisabled]}
                                onPress={nextMatch}
                                activeOpacity={0.7}
                                disabled={!hasResults}
                            >
